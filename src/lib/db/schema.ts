...
export const startups = mysqlTable(
  'chs_hack_startups',
  {
    ...
    productTypes: json('product_types').$type<string[]>().notNull(),
    technologies: json('technologies').$type<string[]>().notNull(),
    regulatoryStatus: varchar('regulatory_status', { length: 50 }),
    needsClinicalTrials: boolean('needs_clinical_trials').default(false),
    nihFundingInterest: varchar('nih_funding_interest', { length: 50 }),
    businessNeeds: json('business_needs').$type<string[]>().default([]),
  ...
  table => ({
    ...
    regulatoryStatusIdx: index('regulatory_status_idx').on(table.regulatoryStatus),
  })
);
